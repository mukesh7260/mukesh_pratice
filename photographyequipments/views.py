...
class PhotographyEquipmentView(APIView):
    def post(self,request):
        serializer=PhotographyEquipmentsSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response("Advertise posted succesfully...!!",status=status.HTTP_201_CREATED)
    ...
    def get(self,request):
        ...
        ads=PhotographyEquipments.objects.all()
        ...
        serializer = PhotographyEquipmentsSerializer(ads,many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    

    def put(self,request,ad_id):
        ad=get_object_or_404(PhotographyEquipments,id=ad_id)
        ...
        serializer=PhotographyEquipmentsSerializer(ad, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
